...
parser.add_argument('--electron', dest='electron', action='store_true')
...
with open('run-linux-client.sh', 'w') as f:
    f.write('\n'.join([
        '#!/bin/bash',
        'set -eo pipefail',
        f"./linux-client start --server_url='{args.server_url}' --account_key='{args.account_key}' --uuid='{args.uuid}'",
    ]))
...
with open('install-linux-client.sh', 'w') as f:
    f.write('\n'.join([
        '#!/bin/bash',
        'set -eo pipefail',
        f"./linux-client install --server_url='{args.server_url}' --account_key='{args.account_key}'",
    ]))
